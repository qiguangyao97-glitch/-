            ...
            textFile.writeText(
                buildString {
                    ...
                    if (analysis != null) {
                        ...
                        appendLine("預計時薪=${OrderAnalyzer.formatMoney(analysis.effectiveHourly)} 元/小時")
                        appendLine("元/公里=${OrderAnalyzer.formatMoney(analysis.yuanPerKm)} 元/公里")
                        appendLine("配送數量=${analysis.deliveryCount}")
                        appendLine("同地點配送=${analysis.isSameLocationStack}")
                        appendLine("標籤備註=${analysis.isWhitelisted} ${analysis.matchedWhitelistKeyword} ${analysis.whitelistNote}")
                        appendLine("避雷標籤=${analysis.isBlacklisted} ${analysis.matchedBlacklistKeyword} ${analysis.blacklistNote}")
                        order?.let {
